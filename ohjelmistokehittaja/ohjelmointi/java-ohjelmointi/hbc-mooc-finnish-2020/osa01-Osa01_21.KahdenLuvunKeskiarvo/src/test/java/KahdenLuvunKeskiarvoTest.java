...
@Points("01-21")
public class KahdenLuvunKeskiarvoTest {
    ...
    private void tulostusOikein(String eka, String toka) {
        String syote = eka + "\n" + toka + "\n";
        io.setSysIn(syote);
        KahdenLuvunKeskiarvo.main(new String[]{});
        String[] lines = new String[]{"Syötä ensimmäinen luku!",
            "Syötä toinen luku!",
            "Syötettyjen lukujen keskiarvo on " + (1.0 * Integer.valueOf(eka) + Integer.valueOf(toka)) / 2
        };
    ...
    @Test
    public void luetaanKokonaisLukuja() {
        List<String> koodi = koodi("KahdenLuvunKeskiarvo.java");
        assertTrue("Lue käyttäjältä kokonaislukuja, vinkki: Integer.valueOf(...).", koodi.stream().filter(l -> l.contains("Integer.")).count() > 0);
    }
